          ...
          <div className="space-y-1.5 sm:space-y-2">
            {logs.map((log, i) => (
              <div key={i} className="flex items-start font-mono group animate-in fade-in slide-in-from-left-1">
                ...
                {/* Arrow Column */}
                <span className={`w-4 sm:w-6 shrink-0 font-black text-[10px] sm:text-xs ${
                  log.type === 'command' ? current.accent :
                  log.type === 'request' ? 'text-blue-400' :
                ...
                {/* Text Content */}
                <span className={`text-[10px] sm:text-xs leading-relaxed break-words whitespace-pre-wrap flex-1 ${
